...
`src/navimower-map-card.js` remains the readable cumulative source. `dist/navimower-map-card.js` is the deterministic production build, minified with a pinned Terser version so Home Assistant loads less JavaScript while source review and regression work stay readable.
...
`npm test` is read-only and verifies the single-runtime layout, optimized-dist contract, release metadata, current behavior contracts and historical regressions that still matter.
