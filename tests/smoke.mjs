...
const runtimeUrl = new URL(distMode ? "../dist/navimower-map-card.js" : "../src/navimower-map-card.js", import.meta.url);
...
if (!distMode) assert.ok(sourceText.includes("NAVIMOWER_MAP_CARD_VERSION"));
...
console.log(`Navimower Map Card ${distMode ? "optimized dist" : "source"} smoke tests passed`);
