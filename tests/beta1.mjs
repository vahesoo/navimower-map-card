...
for (const root of ["src", "dist"]) {
  ...
  const stable = readFileSync(`${root}/navimower-map-card-v034.js`, "utf8");
  ...
  assert.doesNotMatch(loader, /navimower-map-card-v034b7\.js/);
  ...
  assert.match(stable, /NAVIMOWER_MAP_CARD_V034_VERSION = "0\.3\.0"/);
  assert.match(stable, /SCHEDULE_CLOSE_DELAY_MS = 2500/);
  assert.doesNotMatch(stable, /normalizeHexColor|navimower_hex|Object\.assign\(config/);
}
console.log("0.3.0 stable feature checks passed");
