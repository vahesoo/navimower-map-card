...
for (const root of ["src", "dist"]) {
  const beta1Loader = readFileSync(`${root}/navimower-map-card-0.3.1-b1.js`, "utf8");
  const layer = readFileSync(`${root}/navimower-map-card-v035n.js`, "utf8");
  assert.match(beta1Loader, /NAVIMOWER_MAP_CARD_VERSION = "0\.3\.1-beta1"/);
  assert.match(beta1Loader, /navimower-map-card-v035n\.js/);
  assert.doesNotMatch(beta1Loader, /navimower-map-card-v036n\.js/);
  assert.match(beta1Loader, /0\.3\.1-b1/);
  assert.match(beta1Loader, /stable release will use 0\.3\.1\.js/);
