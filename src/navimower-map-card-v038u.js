 ...
 * with one compact, vertically scrollable list. Users choose how many recent
 * notifications to show; refreshed Home Assistant sensor state stays authoritative.
 */
...
export function notificationCount(config = {}) {
  const source = config?.notification_count ?? config?.notification_page_size;
  const value = Math.floor(Number(source));
...
export function normalizeScrollableNotificationConfig(config = {}) {
  return {
    ...(config || {}),
...
export function enforceTwoRowHeader(card) {
  ...
  // Stable core _renderShell() writes display:flex inline on this parent.
  // Inline block restores the intended title row + action row layout.
...
function removeSchemaField(node, name) {
  ...
  if (Array.isArray(node.schema)) {
    node.schema = node.schema.filter((child) => child?.name !== name);
    for (const child of node.schema) removeSchemaField(child, name);
  }
}

export function extendScrollableNotificationConfigForm(form) {
...
function ensureBeta4Styles(card) {
  ...
  style.textContent += `
    ...
    .nm-notification-body { max-height: min(70vh, 650px); overflow-y: auto; overscroll-behavior: contain;
...
function patchCard() {
  ...
  Card.getStubConfig = function beta4StubConfig() {
    const config = normalizeScrollableNotificationConfig(originalStubConfig?.() || {});
    delete config.notification_page_size;
    return config;
  };
  ...
  Card.getConfigForm = function beta4ConfigForm() {
    return extendScrollableNotificationConfigForm(originalConfigForm?.() || { schema: [] });
  };
  ...
  if (typeof originalSetConfig === "function") {
    proto.setConfig = function beta4SetConfig(config) {
      const result = originalSetConfig.call(this, normalizeScrollableNotificationConfig(config));
