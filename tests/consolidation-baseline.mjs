...
assert.ok(source.length <= 826475, `runtime grew during consolidation: ${source.length} chars`);
...
assert.ok(setConfigWrappers.length <= 14, `setConfig wrapper count grew: ${setConfigWrappers.length}`);
assert.ok(ensureDomWrappers.length <= 11, `_ensureDom wrapper count grew: ${ensureDomWrappers.length}`);
