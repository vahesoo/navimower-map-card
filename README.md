...
- **Visual editor** — grouped Displayed information, Appearance, Colors, Map underlay, LiDAR overlay, Notifications, Schedule and Quick settings controls. LiDAR controls are shown only when the integration advertises LiDAR terrain support, or when an existing LiDAR overlay selection must remain editable.
